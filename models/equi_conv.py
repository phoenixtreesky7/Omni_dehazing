...
def equi_conv2d(input, weight, bias=None, stride=(1, 1), padding=(0, 0), dilation=(1, 1)):
    ...
    def rotation_matrix(axis, theta):
        ...
        """
        axis = torch.as_tensor(axis, device=torch.device('cuda'), dtype=input.dtype)
        ...
        ROT = torch.tensor([[aa + bb - cc - dd, 2 * (bc + ad), 2 * (bd - ac)],
                         ...
                         [2 * (bd + ac), 2 * (cd - ab), aa + dd - bb - cc]], device=torch.device('cuda'), dtype=input.dtype)
        return ROT
    ...
    def equi_coord(pano_W,pano_H,k_W,k_H,u,v): 
        ...
        h_range = torch.tensor(range(k_H), device=torch.device('cuda'), dtype=input.dtype)
        w_range = torch.tensor(range(k_W,), device=torch.device('cuda'), dtype=input.dtype)
        w_ones = (torch.ones(k_W, device=torch.device('cuda'), dtype=input.dtype))
        h_ones = (torch.ones(k_H, device=torch.device('cuda'), dtype=input.dtype))
        ...
        K = torch.tensor([[focal,0,c_x],[0,focal,c_y],[0.,0.,1.]], device=torch.device('cuda'), dtype=input.dtype)
        inv_K = torch.inverse(K)
        rays = torch.stack([w_grid,h_grid,torch.ones(h_grid.shape, device=torch.device('cuda'), dtype=input.dtype)],0)
    ...
    def distortion_aware_map(pano_W, pano_H, k_W, k_H, s_width = 1, s_height = 1,bs = 16):
        """ code by cfernandez and jmfacil """
        #n=1
        offset = torch.zeros(2*k_H*k_W,pano_H,pano_W, device='cuda', dtype=input.dtype)
    ...
    return deform_conv2d(input.cuda(), offset, weight, bias=bias, stride=stride, padding=padding, dilation=dilation)
...
class EquiConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0,
        ...
        self.weight = Parameter(torch.empty(out_channels, in_channels // groups,
                                            self.kernel_size[0], self.kernel_size[1])).cuda()

        if bias:
            self.bias = Parameter(torch.empty(out_channels)).cuda()
        else:
    ...
    def forward(self, input):
        ...
        Arguments:
            input (Tensor[batch_size, in_channels, in_height, in_width]): input tensor
        """
        return equi_conv2d(input.cuda(), self.weight, self.bias, stride=self.stride,
                             padding=self.padding, dilation=self.dilation).cuda()
